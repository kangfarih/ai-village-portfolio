...
# providers:
#
#   groq -> gemini -> cline -> ollama -> openrouter -> opencode      (override with LLM_PROVIDER_ORDER)
#
...
# active provider supports it (groq does; gemini/cline/ollama/openrouter/opencode
# do not). If a provider rejects the parameter with HTTP 400/422 whose message
# mentions `reasoning_effort`, the same (provider,key,model) is retried once
# without it.
#
# OpenCode's free models are currently rejected by the provider with HTTP 400 for
# non-OpenCode clients, so that provider typically fails through; it is kept as a
...
# Resolve the active provider order: default all six; LLM_PROVIDER_ORDER may
# be space- or comma-separated, is a subset/reorder, and unknown names are
# ignored with a warning.
ORDER_RAW="${LLM_PROVIDER_ORDER:-groq gemini cline ollama openrouter opencode}"
ORDER_RAW="${ORDER_RAW//,/ }"
...
[ "$ANY_KEY" = "true" ] || fail "no API key configured (set GROQ_API_KEY / GEMINI_API_KEY / CLINE_API_KEY / OLLAMA_API_KEY / OPENROUTER_API_KEY / OPENCODE_API_KEY)"
