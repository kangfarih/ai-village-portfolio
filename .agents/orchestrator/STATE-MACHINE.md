...
`llm_json.sh` tries an ordered chain of OpenAI-compatible providers. Each
provider has a single key; models fall through within a key; providers are tried
...
The default provider order is `openrouter gemini groq cline ollama`
(i.e. `openrouter -> gemini -> groq -> cline -> ollama`).
...
`OLLAMA_ENDPOINT` / `OPENROUTER_ENDPOINT`; model lists via `GROQ_MODELS` /
`GEMINI_MODELS` / `CLINE_MODELS` / `OLLAMA_MODELS` / `OPENROUTER_MODELS`
(space-separated); the order/subset via `LLM_PROVIDER_ORDER`. HTTP 400/404/422
falls through to the next model while 401/402/403 advances to the next
provider. The old single-model `vars.MODEL` repository variable is **no longer
used** — each provider has its own model list.
